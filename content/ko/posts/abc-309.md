...
그리고 선분에서 했던 것 처럼 여기서는 `높이(h)` 에 대해서 오름차순으로 정렬해보면, %%h_i <= h_j%%의 관계가 성립하기 때문에 j의 입장에서 %%\forall 1 \text{\textasciitilde} j-1%% 까지의 도형중에서 `너비(w)`가 %%w_j%% 보다 작은 것이 있는지 찾는 문제로 바뀝니다. 만약 너비에 대해서도 정렬이 되어 있다면 이분 탐색으로 처리할 수 있습니다.

일단 여기까지가 제가 스스로 풀이하면서 생각해본 접근이었구요. 이를 3차원으로 확장하고 났을 때는 도저히 이분탐색으로 풀이할 수 있는 방법이 떠오르진 않았습니다. 그래서 [Editorial](https://atcoder.jp/contests/abc309/editorial/6758)을 참고했습니다. 정답은 N의 크기를 가지는 최소값을 갱신하는 세그먼트 트리를 준비해 둡니다. 이때, 좌표값의 최댓값이 %%10^9%% 이긴 하지만, 입력값의 크기가 %%10^5%%이기 때문에 적절히 **좌표 압축**을 해줍니다. 
